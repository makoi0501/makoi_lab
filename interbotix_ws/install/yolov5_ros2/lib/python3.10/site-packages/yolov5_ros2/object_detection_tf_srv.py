...
import numpy as np
import math
import tf_transformation as tft
...
class ObjectDetection(Node):
    ...
    def __init__(self, **args):
        ...
        self.running = True  # 物体認識の処理のフラグ
        self.target_name = 'sports ball'  # 探す物体名
        ...
        self.service = self.create_service(
            TargetPos, 'target_pos', self.command_callback,
            callback_group=self.callback_group)

    # StringCommandのリクエストが届いたときに呼び出されるメソッド
    def command_callback(self, request, response):
        self.get_logger().info(f'command: {request.target_num}')
