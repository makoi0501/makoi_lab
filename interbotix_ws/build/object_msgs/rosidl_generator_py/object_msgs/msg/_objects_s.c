...
bool object_msgs__msg__objects__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("object_msgs.msg._objects.Objects", full_classname_dest, 32) == 0);
  }
  object_msgs__msg__Objects * ros_message = _ros_message;
  {  // header
  ...
  {  // objects_vector
    ...
    if (!object_msgs__msg__Object__Sequence__init(&(ros_message->objects_vector), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create object_msgs__msg__Object__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    object_msgs__msg__Object * dest = ros_message->objects_vector.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!object_msgs__msg__object__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * object_msgs__msg__objects__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("object_msgs.msg._objects");
  ...
  {  // objects_vector
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = object_msgs__msg__object__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
